...
router.post("/post/:postid/comment", authenticate, async (req, res) => {
  try {
    ...
    const commentedBy = req.author;

    const { text } = req.body;

    const newComment = new Comment({ commentedBy, text });
...
router.get("/allposts", authenticate, isAdmin, async (req, res) => {
  try {
    const allposts = await postSchema
      .find()
      .populate({ path: "author", select: "userId firstName" })
      .populate({
        path: "comments",
        select: "commentedBy text",
        populate: { path: "commentedBy", select: "firstName" },
      });
